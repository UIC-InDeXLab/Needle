...
const StatusPage = () => {
  ...
  useEffect(() => {
    loadAllStatus();
    // Auto-refresh every 2 seconds to show indexing progress
    const interval = setInterval(loadAllStatus, 2000);
  ...
  const loadAllStatus = async () => {
    try {
      ...
      const [
        ...
        mockApi.getHealth(),
        mockApi.getServiceStatus(),
        mockApi.getDirectories(),
        mockApi.getGenerators(),
        mockApi.getSearchLogs().catch(() => ({ data: { queries: [] } })) // Handle search logs error gracefully
      ]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          ...
          ) : status.directories && status.directories.length > 0 ? (

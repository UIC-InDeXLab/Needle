...
const DirectoryPage = () => {
  ...
  const loadDirectoryDetails = async (id) => {
    try {
      ...
      const response = await getDirectory(id);
  ...
  const handleAddDirectory = async (e) => {
    ...
    try {
      ...
      setError(null);
      await addDirectory(newDirectoryPath);
      setNewDirectoryPath('');
  ...
  const handleToggleDirectory = async (id, isEnabled) => {
    try {
      await updateDirectory(id, { is_enabled: !isEnabled });
      await loadDirectories();
  ...
  const handleRemoveDirectory = async (path) => {
    ...
    try {
      ...
      await loadDirectories();
      if (selectedDirectory && directoryDetails?.directory?.path === path) {
        ...
        <div className="card">
          ...
          <form onSubmit={handleAddDirectory} className="space-y-4">
            ...
            <div className="flex space-x-3">
              <button
                type="submit"
                disabled={addingDirectory || !newDirectoryPath.trim()}
                className="btn btn-primary flex items-center"
              >
                {addingDirectory ? (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2 ml-4">
                      <button
                        ...
                        title={directory.is_enabled ? 'Disable' : 'Enable'}
                      >
                        {directory.is_enabled ? (

...
def convert_images_to_embeddings(images):
    embeddings = {}
    with torch.no_grad():
        for image_name, image in tqdm(images):
...
async def startup_event():
    global embeddings
    embeddings_path = Path("embeddings/lvis/embeddings_clip.pkl")

    if embeddings_path.exists():
        # Load old_embeddings from file
        with open(embeddings_path, "rb") as f:
            embeddings = pickle.load(f)
        print(f"Loaded old_embeddings from {embeddings_path}")
    else:
        # Load images
        images = load_images_from_directory(RESOURCES_DIR)
        print(f"Loaded {len(images)} images from {RESOURCES_DIR}")

        # Convert images to old_embeddings
        embeddings = convert_images_to_embeddings(images)
        print(f"Converted images to old_embeddings")

        # Store old_embeddings
        embeddings_path.parent.mkdir(parents=True, exist_ok=True)
        with open(embeddings_path, "wb") as f:
            pickle.dump(embeddings, f)
        print(f"Stored old_embeddings to {embeddings_path}")
